...
flowchart TD
  subgraph Presentation
    A["GraphQL API"] ~~~ B["REST API"] ~~~ C["WebSocket"]
  end
  subgraph Application
    D["Use Cases"] ~~~ E["DTOs"] ~~~ F["Event Handlers"]
  end
  subgraph Domain
    G["Entities"] ~~~ H["Value Objects"] ~~~ I["Domain Services"]
  end
  subgraph Infrastructure
    J["Blockchain Nodes"] ~~~ K["Kafka"] ~~~ L["PostgreSQL"] ~~~ M["Redis"]
  end
...
```mermaid
flowchart LR
  subgraph AnalyzeWalletUseCase
    direction TB
    Input["Input: WalletAddress\nAnalysisDepth 1-10 hops"]
    S1["1. Resolve address → Wallet\n(clustering)"]
    S2["2. Fetch transaction history\n(blockchain indexer)"]
    S3["3. Trace fund flows\n(graph traversal)"]
    S4["4. Score risk\n(rule engine + ML model)"]
    S5["5. Generate attribution\n(entity matching)"]
    Output["Output: WalletAnalysis\n(risk score, attribution, fund flow graph)"]
...
```mermaid
flowchart LR
  BTC["Bitcoin Node"] --> BL1["BTC Block Listener"] --> K1["Kafka: raw-blocks-btc"]
  ETH["Ethereum Node"] --> BL2["ETH Block Listener"] --> K2["Kafka: raw-blocks-eth"]
  MATIC["Polygon Node"] --> BL3["MATIC Block Listener"] --> K3["Kafka: raw-blocks-matic"]
```
...
```mermaid
flowchart TD
  RB["raw-blocks"] --> BP["Block Parser"] --> PT["parsed-transactions"]
  PT --> TI["Transaction Indexer"]
  PT --> AC["Address Clusterer"]
  PT --> RS["Risk Scorer"]
  TI --> DB1[("PostgreSQL\ntransactions")]
  AC --> DB2[("PostgreSQL\nclusters")]
  RS --> DB3[("PostgreSQL\nrisk_scores")]
